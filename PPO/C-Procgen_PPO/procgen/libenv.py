...
class CEnv(Env):
    ...
    def __init__(
        ...
        # On Windows, add likely runtime DLL directories (env.dll folder + Qt bin)
        # so cffi can resolve dependent libraries like Qt5Gui.dll.
        self._dll_dirs = []
        if platform.system() == "Windows" and hasattr(os, "add_dll_directory"):
            dll_candidates = [
                lib_dir,
                r"C:\Qt\5.15.2\msvc2019_64\bin",
                r"C:\Qt\5.15.2\msvc2022_64\bin",
            ]
            for dll_dir in dll_candidates:
                if os.path.isdir(dll_dir):
                    self._dll_dirs.append(os.add_dll_directory(dll_dir))
    ...
    def _convert_contexts(self, ffi: Any, c_lib: Any, context_options: List) -> Any:
        ...
        if context_options is None:
            context_options = [{} for _ in range(self.num)]
        env_num = len(context_options)
        if env_num != self.num:
            raise ValueError(
                f"context_options length mismatch: expected {self.num}, got {env_num}"
            )
    ...
    def get_context(self) -> Tuple[Dict[str, Any]]:
        ...
        for env_idx in range(self.num):
            env_data = self._c_buffers[0].explicit_contexts[env_idx]
            env_context_num = env_data.count
            for i in range(env_context_num):
                context = env_data.items[i]
                key = self._ffi.string(context.name).decode("utf-8")
