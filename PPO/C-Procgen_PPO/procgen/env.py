...
class BaseProcgenEnv(CEnv):
    ...
    def __init__(
        ...
        # Ensure we have one context entry per env instance.
        # If fewer are provided, repeat them round-robin.
        if context_options is None:
            context_options = [{} for _ in range(num)]
        elif len(context_options) == 0:
            raise ValueError("context_options must contain at least one item")
        elif len(context_options) != num:
            context_options = [context_options[i % len(context_options)] for i in range(num)]

        # Combine default context options with user-specified ones.
        new_context_options = []
