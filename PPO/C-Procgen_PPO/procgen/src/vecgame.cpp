...
libenv_env *libenv_make(int num_envs, const struct libenv_options options) {
    std::vector<VecOptions> contexts_options(num_envs, VecOptions(options));
    auto venv = new VecGame(num_envs, VecOptions(options), contexts_options.data());
...
libenv_env *libenv_contextual_make(int num_envs, const struct libenv_options options, const struct libenv_options contexts[]) {
    std::vector<VecOptions> contexts_options(num_envs);
...
VecGame::VecGame(int _nenvs, VecOptions opts, VecOptions contexts[]) {
    ...
    {
        struct libenv_tensortype s;
        strcpy(s.name, "death_type");
        s.scalar_type = LIBENV_SCALAR_TYPE_DISCRETE;
        s.dtype = LIBENV_DTYPE_UINT8;
        s.ndim = 0,
        s.low.uint8 = DeathNone;
        s.high.uint8 = DeathTimeout;
        info_types.push_back(s);
    }
