...
class CoinRun : public BasicAbstractGame {
  public:
    ...
    void handle_agent_collision(const std::shared_ptr<Entity> &obj) override {
        ...
        if (obj->type == ENEMY) {
            // step_data.reward += DEATH_PENALTY;
            step_data.done = true;
            step_data.death_type = DeathByEnemy;
        } else if (obj->type == SAW) {
            // step_data.reward += DEATH_PENALTY;
            step_data.done = true;
            step_data.death_type = DeathBySaw;
        }
    }
    ...
    void handle_grid_collision(const std::shared_ptr<Entity> &obj, int type, int i, int j) override {
        if (obj->type == PLAYER) {
            if (type == GOAL) {
                ...
                // step_data.reward += DEATH_PENALTY;
                step_data.done = true;
                step_data.death_type = DeathByLava;
            }
        }
    }
    ...
    void generate_coin_to_the_right() {
        ...
        int num_sections = rand_gen.randn(coinrun_context_option->max_section_num - coinrun_context_option->min_section_num + 1) + coinrun_context_option->min_section_num;
