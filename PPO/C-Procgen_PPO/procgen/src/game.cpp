...
Game::Game(std::string name) : game_name(name) {
    ...
    step_data.death_type = DeathNone;
...
void Game::step() {
    ...
    if (step_data.done && (cur_time >= timeout) && step_data.death_type == DeathNone) {
        step_data.death_type = DeathTimeout;
    }

    if (step_data.done && !step_data.level_complete && !will_force_reset && step_data.death_type == DeathNone) {
        step_data.death_type = DeathUnknown;
    }
...
void Game::observe() {
    ...
    *(uint8_t *)(info_bufs[info_name_to_offset.at("death_type")]) = (uint8_t)(step_data.death_type);
}
...
void Game::serialize(WriteBuffer *b) {
    ...
    b->write_int(step_data.death_type);
...
void Game::deserialize(ReadBuffer *b) {
    ...
    step_data.death_type = DeathType(b->read_int());
