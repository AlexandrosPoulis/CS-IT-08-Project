...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="PB2-style population search wrapper for ppo.py")
...
def collect_member(handle: dict, state: dict, state_path: Path, output_root: Path) -> None:
...
def run_manager(args: argparse.Namespace) -> None:
    ...
    while True:
        active_members = [member for member in state["members"] if member["timesteps"] < args.total_timesteps]
        if not active_members:
            break

        round_index = state["round_index"]
        running = {}
        gpu_pool = list(range(min(args.parallel_agents, args.gpus)))
        ...
        while queue or running:
            ...
            for gpu_id, handle in list(running.items()):
                if handle["process"].poll() is None:
                    continue
                collect_member(handle, state, state_path, output_root)
                gpu_pool.append(gpu_id)
                del running[gpu_id]

        state["round_index"] += 1
        if any(member.get("status") == "failed" for member in state["members"]):
            break
...
def build_or_load_model(train_env, round_dir: Path, params: dict, checkpoint_path: str):
    ...
    if checkpoint and checkpoint.exists():
        model = PPO.load(str(checkpoint), env=train_env, device="cuda")
...
def run_worker(args: argparse.Namespace) -> None:
    ...
    model, resumed = build_or_load_model(train_env, round_dir, params, args.checkpoint_path)
    try:
        model.learn(
