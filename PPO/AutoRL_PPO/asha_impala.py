...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ASHA search wrapper for main_impala.py")
...
def render_worker(trial_dir: Path, params: dict, timesteps: int, base_module: str, env_name: str) -> Path:
    ...
    worker_path = trial_dir / "worker_impala.py"
...
model.save(str(trial_dir / "impala"))
...
summary = {{
    ...
    "model_path": str((trial_dir / "impala.zip").resolve()),
...
def run_manager(args: argparse.Namespace) -> None:
    ...
    base_module = state.get("base_module", args.base_module)
    env_name = state.get("env_name", args.env_name)
    ...
    for stage_index in range(len(STAGE_TIMESTEPS)):
        ...
        while queue or running:
            while queue and available_gpus:
                ...
                save_state(state, state_path)
                running[gpu_id] = launch_trial(trial, output_root, stage_index, gpu_id, base_module, env_name)
